...
# Use @api_bp.route decorators. Import:
#   - request, jsonify (from flask)
#   - api_bp from .
#   - require_jwt, require_api_key from decorators
#   - get_profile_doc_ref, get_profile_data, set_profile from utils.profile
#   - validate_profile_data, normalize_profile_data, require_json_content_type from utils.validation
import time
...
def api_get_sensor_data(uid: str):
    """Return mock sensor data from local JSON file."""
    sensor_file = os.path.join(os.path.dirname(__file__), '..', '..', 'mock_sensor_data.json')
    try:
        with open(sensor_file, 'r') as f:
            sensor_data = json.load(f)
    except Exception as e:
        return jsonify({"error": f"Failed to load sensor data: {str(e)}"}), 500
